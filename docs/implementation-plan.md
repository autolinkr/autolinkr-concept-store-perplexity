...
| C5 | Configure aReviews visibility rules per product page + connect Klaviyo review request email (Flow 3, Day 10) | +100 reviews/month; full visibility control | 2 hours |
...
### 12.8 aReviews — Product Page Review Visibility

**Role in the stack:** aReviews is the review display layer on all product pages. It is already installed and is the single source of truth for what review content visitors see on each product page. Unlike apps like Judge.me or Loox that display all reviews automatically, aReviews gives full manual control over which individual reviews are visible on each product — essential for a catalog where some products have hundreds of reviews (Pro: 916) and others are sparse (Elite: 42, Dash™: early stage).

**Why visibility control matters for AutoLinkr specifically:**

| Product | Review situation | Strategy |
|---|---|---|
| AutoLinkr Pro™ | 916 reviews — strong foundation | Show the highest-quality reviews above fold. Curate for photo reviews, detailed setups, compatibility callouts. Hide one-liners that add no conversion value. |
| AutoLinkr Elite™ | 42 reviews — thin coverage | Surface every high-quality Elite review prominently. Supplement below with a "Also loved by AutoLinkr Pro owners" section if the platform allows cross-product display. |
| AutoLinkr™ Base | 409 reviews | Curate reviews that specifically mention wireless convenience (the Base value prop). Hide reviews that mention streaming — those belong on the Pro page. |
| AutoLinkr Dash™ | Early stage — few reviews | Show all available reviews. Once volume grows, curate for safety-specific praise (night vision, G-sensor catches). |
| AutoLinkr Tablet™ | Low volume | Show all. Prioritize reviews from non-CarPlay-car owners — those are the most persuasive for the Tablet™ audience. |
| MagSafe Mount | 2,543 units sold — reviews available | Surface reviews that mention it working with AutoLinkr specifically ("perfect with my Pro"). |

**Integration with Klaviyo review requests:**
- Klaviyo Flow 3, Email 4 (Day 10 post-purchase) fires a review request to all buyers
- aReviews receives the submitted review and holds it for visibility review before it appears on the product page
- This means: all review requests go through Klaviyo, all display decisions are made in aReviews
- Do NOT use a separate review request trigger inside aReviews — keep Klaviyo as the single trigger to avoid duplicate requests

**aReviews — no dedicated nav page:**
Per the navigation architecture in Section 4.2, there is no standalone `/pages/reviews` page. Reviews live on the product page where they support the buying decision. aReviews is configured per-product-page only.
...
| **aReviews** | Product page reviews — full visibility control (show/hide individual reviews per product) | Installed |
| **Rebuy** | AI Smart Cart, post-purchase upsell offers, product page widgets | ~$99/mo |
| Tidio or Gorgias | Live chat + AI chatbot | $29–$60/mo |
| **Jebbit** | Product selector quizzes (Adapter Selector + Compatibility Checker) + Klaviyo lead sync | Installed |
| Smile.io | Loyalty & referral program | $49/mo |
| **Recharge** | VIP Membership subscription billing + Klaviyo integration | ~$99/mo |
